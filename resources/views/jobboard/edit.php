...
<div class="bg-gray-50 min-h-screen py-8 pb-32">
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            ...
            <form action="<?= APP_URL ?>/jobs/edit/<?= $job['id'] ?>" method="POST" id="job-form" enctype="multipart/form-data">
                <input type="hidden" name="csrf_token" value="<?= e($_SESSION['csrf_token'] ?? '') ?>">
                <input type="hidden" name="source" value="<?= e($_GET['source'] ?? 'dashboard') ?>">
                    ...
                    <div class="px-6 py-5">
                        <div class="grid grid-cols-2 sm:grid-cols-3 gap-3" id="category-grid">
                            <?php foreach ($categories as $catName => $catData):
                                $currentCat = $_POST['category'] ?? $job['service_category'] ?? '';
...
function handleFiles(fileList) {
    ...
    for (let f of fileList) {
        if (selectedFiles.length + existingImages.length >= MAX_IMAGES) {
            errText.textContent = 'Maximum ' + MAX_IMAGES + ' images allowed combined. Please delete an existing image to make space.';
            errEl.classList.remove('hidden');
...
function removeExistingImage(path) {
    ...
    // Add hidden input to tell the backend to delete it on save
    const input = document.createElement('input');
    input.type = 'hidden';
    input.name = 'delete_images[]';
    input.value = path;
    document.getElementById('job-form').appendChild(input);
    
    // Clear any errors that might be blocking interaction
...
function renderPreviews() {
    ...
    existingImages.forEach((imgSrc) => {
        ...
        btn.innerHTML = '<svg class="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg>';
        btn.onclick = () => removeExistingImage(imgSrc);
    ...
    selectedFiles.forEach((file, idx) => {
        ...
        btn.className = 'absolute top-1.5 right-1.5 h-6 w-6 rounded-full bg-red-500 hover:bg-red-600 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-1 text-white flex items-center justify-center shadow-lg transition-colors';
        btn.innerHTML = '<svg class="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg>';
        btn.onclick = () => removeNewImage(idx);
