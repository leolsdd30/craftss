    ...
    <div class="w-full lg:w-1/2 min-h-[calc(100dvh-4rem)] flex items-center justify-center px-6 py-12 sm:px-12">
        <div class="w-full max-w-md">
            ...
            <div class="text-center">
                ...
                <h1 class="text-2xl font-extrabold text-gray-900 dark:text-white mb-2"><?= __('auth_forgot.success_title') ?></h1>
                <p class="text-sm text-gray-500 dark:text-gray-400 mb-8">
                    <?= __('auth_forgot.success_desc') ?>
                </p>
                ...
                <div class="mb-6 bg-amber-50 dark:bg-amber-900/30 border border-amber-200 dark:border-amber-800 rounded-xl p-5 text-left">
                    <div class="flex items-center space-x-2 mb-3">
                        ...
                        <p class="text-xs font-bold text-amber-700 dark:text-amber-500 uppercase tracking-wide"><?= __('auth_forgot.dev_mode') ?></p>
                    </div>
                    <p class="text-xs text-amber-700 dark:text-amber-500 mb-3"><?= __('auth_forgot.dev_desc') ?></p>
                ...
                <a href="<?= APP_URL ?>/login"
                   class="inline-flex items-center text-sm font-medium text-indigo-600 hover:text-indigo-500 transition">
                    <svg class="mr-1.5 <?= __('lang') === 'ar' ? 'ml-1.5 rotate-180' : 'mr-1.5' ?> h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            ...
            <!-- ── FORM STATE ───────────────────────────────────────── -->
            <div class="mb-8">
                <h1 class="text-3xl font-extrabold text-gray-900 dark:text-white"><?= __('auth_forgot.form_title') ?></h1>
                <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">
                    <?= __('auth_forgot.form_desc') ?>
                </p>
            </div>
            ...
            <form action="<?= APP_URL ?>/forgot-password" method="POST" class="space-y-5">
                ...
                <div>
                    <label for="email" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1"><?= __('auth_forgot.email_label') ?></label>
