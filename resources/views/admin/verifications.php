    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            <div class="flex items-start justify-between">
                <div>
                    <a href="<?= APP_URL ?>/admin/dashboard" class="text-sm font-medium text-indigo-600 dark:text-indigo-400 hover:text-indigo-500 dark:hover:text-indigo-300 transition-colors"><?= __('admin.back_to_dashboard') ?></a>
                    <h1 class="mt-2 text-3xl font-extrabold text-gray-900 dark:text-white"><?= __('admin.craftsman_verification') ?></h1>
                    <p class="text-sm text-gray-500 dark:text-gray-400 mt-1"><?= __('admin.craftsman_verification_desc') ?></p>
                </div>
        ...
        <div class="border-b border-gray-200 dark:border-gray-700 mb-6">
            <nav class="flex gap-8">
                <?php foreach (['pending' => __('admin.pending'), 'verified' => __('admin.verified'), 'all' => __('admin.all')] as $val => $label): ?>
                <a href="<?= APP_URL ?>/admin/verifications?filter=<?= $val ?>"
                   class="<?= $filter === $val ? 'border-indigo-500 text-indigo-600 dark:border-indigo-400 dark:text-indigo-400' : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 hover:border-gray-300 dark:hover:border-gray-600' ?> whitespace-nowrap py-3 px-1 border-b-2 font-medium text-sm transition-colors">
                    ...
                    <span class="ms-2 bg-indigo-100 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-400 text-xs font-bold px-2 py-0.5 rounded-full"><?= $totalCraftsmen ?></span>
                    <?php endif; ?>
                </a>
                <?php endforeach; ?>
            </nav>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-5 mb-6">
            <form method="GET" action="<?= APP_URL ?>/admin/verifications" class="flex flex-wrap items-end gap-4">
                ...
                <div class="flex-1 min-w-[200px]">
                    <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1"><?= __('admin.search_by_name') ?></label>
                    <input type="text" name="search" value="<?= htmlspecialchars($search) ?>" placeholder="<?= __('admin.search_by_name_pl') ?>" autocomplete="off"
                ...
                <div>
                    <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1"><?= __('profile_edit.wilaya') ?></label>
                    <select name="wilaya" class="border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm px-4 py-2 border">
                        ...
                        <option value="<?= htmlspecialchars($w) ?>" <?= $wilayaFilter === $w ? 'selected' : '' ?>>
                            <?= htmlspecialchars(preg_replace('/^\d{2}\s-\s/', '', __('wilayas.' . $w) ?? $w)) ?>
                        </option>
                        <?php endforeach; ?>
                    </select>
                </div>
                ...
                <div>
                    <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1"><?= __('search.category') ?></label>
                    <select name="category" class="border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm px-4 py-2 border">
                        ...
                        <option value="<?= htmlspecialchars($cat) ?>" <?= $categoryFilter === $cat ? 'selected' : '' ?>><?= __('categories.' . $cat) ?? htmlspecialchars($cat) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <?php endif; ?>
                ...
                <div>
                    <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1"><?= __('admin.sort_by') ?></label>
                    <select name="sort" class="border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm px-4 py-2 border">
                        <option value="date_desc" <?= ($sortFilter ?? 'date_desc') === 'date_desc' ? 'selected' : '' ?>><?= __('admin.newest_first') ?></option>
                        <option value="date_asc"  <?= ($sortFilter ?? '') === 'date_asc'  ? 'selected' : '' ?>><?= __('admin.oldest_first') ?></option>
                        <option value="name_asc"  <?= ($sortFilter ?? '') === 'name_asc'  ? 'selected' : '' ?>><?= __('admin.name_az') ?></option>
                        <option value="name_desc" <?= ($sortFilter ?? '') === 'name_desc' ? 'selected' : '' ?>><?= __('admin.name_za') ?></option>
                    </select>
                </div>
                ...
                <div class="flex items-center gap-3">
                    <button type="submit" class="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-lg text-white bg-indigo-600 dark:bg-indigo-500 hover:bg-indigo-700 dark:hover:bg-indigo-600 transition duration-150">
                        <svg class="me-2 h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
                        <?= __('admin.filter_btn') ?>
                    </button>
                    <?php if (!empty($search) || !empty($wilayaFilter) || !empty($categoryFilter) || !empty($sortFilter)): ?>
                    <a href="<?= APP_URL ?>/admin/verifications?filter=<?= htmlspecialchars($filter) ?>" class="text-sm text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 transition-colors"><?= __('admin.clear') ?></a>
                    <?php endif; ?>
                </div>
            </form>
        </div>
        ...
        <div class="rounded-lg bg-green-50 dark:bg-green-900/30 p-4 mb-6 border border-green-200 dark:border-green-800">
            <div class="flex">
                ...
                <p class="ms-3 text-sm font-medium text-green-800 dark:text-green-300"><?= __('admin.verification_success') ?></p>
            </div>
        </div>
        <?php endif; ?>
        ...
        <div class="flex flex-wrap items-center justify-between gap-3 mb-4">
            <p class="text-sm text-gray-500 dark:text-gray-400">
                <?= __('admin.showing') ?> <span class="font-semibold text-gray-700 dark:text-gray-300"><?= count($craftsmen) ?></span>
                <?= __('admin.of') ?> <span class="font-semibold text-gray-700 dark:text-gray-300"><?= $totalCraftsmen ?></span>
                <?= $totalCraftsmen !== 1 ? __('admin.craftsman_plural') : __('admin.craftsman_singular') ?>
            </p>
            <?php if ($totalPages > 1): ?>
            <nav class="isolate inline-flex -space-x-px rounded-md shadow-sm">
                <?php if ($page > 1): ?>
                <a href="?<?= $qs ?>&page=<?= $page - 1 ?>" class="relative inline-flex items-center rounded-l-md rtl:rounded-l-none rtl:rounded-r-md px-2 py-1.5 text-gray-400 dark:text-gray-500 ring-1 ring-inset ring-gray-300 dark:ring-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700">
                ...
                <a href="?<?= $qs ?>&page=<?= $page + 1 ?>" class="relative inline-flex items-center rounded-r-md rtl:rounded-r-none rtl:rounded-l-md px-2 py-1.5 text-gray-400 dark:text-gray-500 ring-1 ring-inset ring-gray-300 dark:ring-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl border-2 border-dashed border-gray-200 dark:border-gray-700 p-12 text-center">
            ...
            <h3 class="text-lg font-medium text-gray-900 dark:text-white"><?= __('admin.no_craftsmen_found') ?></h3>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400"><?= $filter === 'pending' ? __('admin.no_pending_verifications') : __('admin.no_craftsmen_match') ?></p>
        </div>
        <?php else: ?>
                ...
                <div class="p-6">
                    ...
                    <div class="flex items-center gap-4 mb-4">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-xs text-gray-400 dark:text-gray-500 mt-0.5 flex items-center">
                                <svg class="h-3 w-3 me-1 flex-shrink-0" viewBox="0 0 20 20" fill="currentColor">
                    ...
                    <div class="space-y-2 mb-4">
                        ...
                        <div class="flex items-center justify-between text-sm">
                            <span class="text-gray-500 dark:text-gray-400"><?= __('search.category') ?></span>
                            <span class="inline-flex items-center px-2 py-0.5 rounded text-[11px] uppercase tracking-wider font-bold <?= $catColor($c['service_category']) ?>"><?= __('categories.' . $c['service_category']) ?? htmlspecialchars($c['service_category']) ?></span>
                        </div>
                        <?php endif; ?>
                        ...
                        <div class="flex items-center justify-between text-sm">
                            <span class="text-gray-500 dark:text-gray-400"><?= __('admin.status_col') ?></span>
                            <?php if ($c['is_verified']): ?>
                            ...
                            <?php else: ?>
                            <span class="inline-flex items-center text-xs font-semibold text-yellow-700 dark:text-yellow-400 bg-yellow-50 dark:bg-yellow-900/20 px-2 py-0.5 rounded-full"><?= __('admin.pending') ?></span>
                            <?php endif; ?>
                        </div>
                    </div>
                    ...
                    <form id="verify-form-<?= $c['user_id'] ?>" method="POST" action="<?= APP_URL ?>/admin/verifications/toggle">
                        ...
                        <button type="button"
                                ...
                                onclick="showConfirmModal('verify-form-<?= $c['user_id'] ?>', '<?= __('admin.remove_verification') ?>', '<?= __('admin.msg_rem_1') ?><?= htmlspecialchars($c['first_name']) ?><?= __('admin.msg_rem_2') ?>', 'decline')">
                            <svg class="me-2 h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                            <?= __('admin.remove_verification') ?>
                        </button>
                        <?php else: ?>
                        <button type="button"
                                ...
                                onclick="showConfirmModal('verify-form-<?= $c['user_id'] ?>', '<?= __('admin.verify_craftsman_btn') ?>', '<?= __('admin.msg_ver_1') ?><?= htmlspecialchars($c['first_name']) ?><?= __('admin.msg_ver_2') ?>', 'accept')">
                            <svg class="me-2 h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/></svg>
                            <?= __('admin.verify_craftsman_btn') ?>
                        </button>
                        <?php endif; ?>
                    </form>
        ...
        <div class="mt-8 flex flex-col sm:flex-row items-center justify-between border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 px-4 py-3 sm:px-6 rounded-xl shadow-sm gap-4">
            <p class="text-sm text-gray-700 dark:text-gray-300 hidden sm:block"><?= __('admin.page') ?? 'Page' ?> <span class="font-medium dark:text-white"><?= $page ?></span> <?= __('admin.of') ?? 'of' ?> <span class="font-medium dark:text-white"><?= $totalPages ?></span></p>
            <nav class="isolate inline-flex -space-x-px rounded-md shadow-sm">
                <?php if ($page > 1): ?>
                <a href="?<?= $qs ?>&page=<?= $page - 1 ?>" class="relative inline-flex items-center rounded-l-md rtl:rounded-l-none rtl:rounded-r-md px-2 py-2 text-gray-400 dark:text-gray-500 ring-1 ring-inset ring-gray-300 dark:ring-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700">
                    <svg class="h-5 w-5 <?= __('lang') === 'ar' ? 'rotate-180' : '' ?>" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M12.79 5.23a.75.75 0 01-.02 1.06L8.832 10l3.938 3.71a.75.75 0 11-1.04 1.08l-4.5-4.25a.75.75 0 010-1.08l4.5-4.25a.75.75 0 011.06.02z" clip-rule="evenodd"/></svg>
                </a>
                <?php endif; ?>
                <?php for ($i = max(1, $page - 2); $i <= min($totalPages, $page + 2); $i++): ?>
                ...
                <?php endfor; ?>
                <?php if ($page < $totalPages): ?>
                <a href="?<?= $qs ?>&page=<?= $page + 1 ?>" class="relative inline-flex items-center rounded-r-md rtl:rounded-r-none rtl:rounded-l-md px-2 py-2 text-gray-400 dark:text-gray-500 ring-1 ring-inset ring-gray-300 dark:ring-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700">
                    <svg class="h-5 w-5 <?= __('lang') === 'ar' ? 'rotate-180' : '' ?>" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M7.21 14.77a.75.75 0 01.02-1.06L11.168 10 7.23 6.29a.75.75 0 111.04-1.08l4.5 4.25a.75.75 0 010 1.08l-4.5 4.25a.75.75 0 01-1.06-.02z" clip-rule="evenodd"/></svg>
                </a>
                <?php endif; ?>
            </nav>
        </div>
    ...
    <div class="fixed inset-0 flex items-center justify-center p-4">
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-xl max-w-md w-full p-6">
            <div class="flex items-center mb-4">
                <div id="modal-icon-accept" class="hidden h-10 w-10 rounded-full bg-green-100 dark:bg-green-900/30 flex items-center justify-center me-3">
                ...
                <div id="modal-icon-decline" class="hidden h-10 w-10 rounded-full bg-red-100 dark:bg-red-900/30 flex items-center justify-center me-3">
            ...
            <div class="flex justify-end gap-3">
                <button onclick="hideConfirmModal()" class="px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 rounded-lg transition duration-150"><?= __('admin.cancel') ?></button>
                <button id="modal-confirm-btn" onclick="confirmAction()" class="px-4 py-2 text-sm font-medium text-white rounded-lg transition duration-150"><?= __('admin.confirm') ?></button>
            </div>
        </div>
    </div>
</div>
...
function showConfirmModal(formId, title, message, type) {
    ...
    if (type === 'accept') {
        ...
        btn.textContent = '<?= __('admin.yes_verify') ?>';
        ...
        btn.textContent = '<?= __('admin.yes_remove') ?>';
