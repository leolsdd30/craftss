    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            <div class="flex items-start justify-between">
                <div>
                    ...
                    <h1 class="mt-2 text-3xl font-extrabold text-gray-900 dark:text-white"><?= __('admin.user_management') ?></h1>
                    <p class="text-sm text-gray-500 dark:text-gray-400 mt-1"><?= __('admin.user_management_desc') ?></p>
                </div>
                <a href="<?= APP_URL ?>/admin/verifications"
                   class="mt-6 inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-lg text-white bg-indigo-600 dark:bg-indigo-500 hover:bg-indigo-700 dark:hover:bg-indigo-600 transition duration-150">
                    ...
                    <span class="ms-2 bg-yellow-400 text-yellow-900 border border-yellow-500 text-xs font-bold px-1.5 py-0.5 rounded-full"><?= $stats['pending_verification'] ?></span>
                    <?php endif; ?>
                </a>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-5 mb-6">
            <form method="GET" action="<?= APP_URL ?>/admin/users" class="flex flex-wrap items-end gap-4">
                <div class="flex-1 min-w-[200px]">
                    ...
                    <input type="text" name="search" value="<?= htmlspecialchars($search) ?>" placeholder="<?= __('admin.search_placeholder') ?>" autocomplete="off"
                ...
                <div>
                    <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1"><?= __('admin.role') ?></label>
                    <select name="role" class="border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm px-4 py-2 border">
                        ...
                        <option value="homeowner" <?= $roleFilter === 'homeowner' ? 'selected' : '' ?>><?= __('auth.homeowner') ?? 'Homeowner' ?></option>
                        <option value="craftsman" <?= $roleFilter === 'craftsman' ? 'selected' : '' ?>><?= __('auth.craftsman') ?? 'Craftsman' ?></option>
                        <option value="admin"     <?= $roleFilter === 'admin'     ? 'selected' : '' ?>><?= __('auth.admin') ?? 'Admin' ?></option>
                    </select>
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1"><?= __('admin.status_col') ?></label>
                    <select name="status" class="border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm px-4 py-2 border">
                        ...
                        <option value="active"   <?= $statusFilter === 'active'   ? 'selected' : '' ?>><?= __('admin.active_status') ?></option>
                        <option value="inactive" <?= $statusFilter === 'inactive' ? 'selected' : '' ?>><?= __('admin.inactive_status') ?></option>
                    </select>
                </div>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <a href="<?= APP_URL ?>/admin/users" class="text-sm text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 transition-colors"><?= __('admin.clear') ?></a>
                    <?php endif; ?>
                </div>
            </form>
        </div>
        ...
        <div class="rounded-lg bg-green-50 dark:bg-green-900/30 p-4 mb-6 border border-green-200 dark:border-green-800">
            <div class="flex">
                ...
                <p class="ms-3 text-sm font-medium text-green-800 dark:text-green-300"><?= __('admin.user_status_success') ?></p>
            </div>
        </div>
        <?php endif; ?>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
            ...
            <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex flex-wrap items-center justify-between gap-3">
                <p class="text-sm text-gray-500 dark:text-gray-400">
                    <?= __('admin.showing') ?> <span class="font-semibold text-gray-700 dark:text-gray-300"><?= count($users) ?></span>
                    <?= __('admin.of') ?> <span class="font-semibold text-gray-700 dark:text-gray-300"><?= $totalUsers ?? count($users) ?></span>
                    <?= ($totalUsers ?? count($users)) !== 1 ? __('admin.user_plural') : __('admin.user_singular') ?>
                </p>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead class="bg-gray-50 dark:bg-gray-800/50">
                        <tr>
                            <th class="px-6 py-3 text-start text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">ID</th>
                            ...
                            <th class="px-6 py-3 text-end text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider"><?= __('admin.actions') ?></th>
                        </tr>
                    </thead>
                    <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        <?php if (empty($users)): ?>
                        <tr><td colspan="7" class="px-6 py-12 text-center text-sm text-gray-500 dark:text-gray-400"><?= __('admin.no_users_found') ?></td></tr>
                        <?php endif; ?>
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-xs text-gray-400 dark:text-gray-500 mt-0.5"><?= htmlspecialchars(preg_replace('/^\d{2}\s-\s/', '', __('wilayas.' . $u['wilaya']) ?? $u['wilaya'])) ?></p>
                                        <?php endif; ?>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                ?>
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium <?= $rc ?> capitalize"><?= __('auth.' . $u['role']) ?? htmlspecialchars($u['role']) ?></span>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-end">
                                ...
                                <form id="status-form-<?= $u['id'] ?>" method="POST" action="<?= APP_URL ?>/admin/users/toggle-status" class="inline">
                                    ...
                                    <button type="button" class="text-xs font-medium text-red-600 dark:text-red-400 hover:text-red-800 dark:hover:text-red-300 bg-red-50 dark:bg-red-900/20 hover:bg-red-100 dark:hover:bg-red-900/40 px-3 py-1.5 rounded-lg transition-colors"
                                            onclick="showConfirmModal('<?= __('admin.deactivate_user') ?>', '<?= __('admin.msg_deac_1') ?><?= htmlspecialchars($u['first_name']) ?><?= __('admin.msg_deac_2') ?>', 'decline', 'status-form-<?= $u['id'] ?>')">
                                    ...
                                    <button type="button" class="text-xs font-medium text-green-600 dark:text-green-400 hover:text-green-800 dark:hover:text-green-300 bg-green-50 dark:bg-green-900/20 hover:bg-green-100 dark:hover:bg-green-900/40 px-3 py-1.5 rounded-lg transition-colors"
                                            onclick="showConfirmModal('<?= __('admin.activate_user') ?>', '<?= __('admin.msg_act_1') ?><?= htmlspecialchars($u['first_name']) ?><?= __('admin.msg_act_2') ?>', 'accept', 'status-form-<?= $u['id'] ?>')">
        ...
        <!-- Bottom Pagination -->
        <?php if (isset($totalPages) && $totalPages > 1): ?>
        <div class="mt-6 flex flex-col sm:flex-row items-center justify-between border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 px-4 py-3 sm:px-6 rounded-xl shadow-sm gap-4">
...
function showConfirmModal(title, message, type, formId) {
    ...
    if (type === 'accept') {
        ...
        btn.textContent = '<?= __('admin.yes_activate') ?>';
        ...
        btn.textContent = '<?= __('admin.yes_deactivate') ?>';
